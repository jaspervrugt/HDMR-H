function [SA,SA_sig,Fx] = HDMR_H_end(SA,nterms,p0,RMSE,select,K,C2, ...
    C3,n1,n2,n3,n)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %
%                                                                         %
...
%                                                                         %
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %
% Prepares the return arguments of HDMR_H                                 %
%                                                                         %
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %
%                                                                         %
%  SYNOPSIS                                                               %
%   [SA,SA_sig,Fx] = HDMR_H_end(SA,nterms,p0,RMSE,select,K,C2, ...        %
%       C3,n1,n2,n3,n)                                                    %
%                                                                         %
...
% Skip line with HDMR_H print statement about progress
fprintf('\n');
% Set the alfa value of boostrap confidence intervals
alfa = 0.95;
...
nr = size(SA,1); idx_sig = [ 1 ; 1 + find( f_ret > 0) ; nr ]; SA_sig = SA(idx_sig,1:13);
...
fid = fopen('HDMR_H_results.txt','w+');
fprintf(fid,'|-------------------------------------------------------------------------|\n');
fprintf(fid,'|                                                                         |\n');
...
if K == 1
    fprintf(fid,['Table 1: Properties of HDMR_H emulator, y = f(x), ' ...
...
% Print to file the results of the HDMR_H analysis
for pr_tab = 1:2
    ...
    if K == 1
        if pr_tab == 1
            fprintf(fid,['Table 2: HDMR_H results for all model ' ...
                'components using all X and Y data ' ...
                '(no bootstrapping)\n']);
        else
            fprintf(fid,['Table 3: HDMR_H results for significant ' ...
                'model components only using all X and Y data ' ...
                '(no bootstrapping)\n']);
        end
    elseif K > 1
        if pr_tab == 1
            ...
            fprintf(fid,['Table 3: HDMR_H results for significant ' ...
                'model components only using %d bootstrap trials\n'],K);
        end
    end
    % Now print table
    fprintf(fid,'==============================================================================================\n');
    ...
    fprintf(fid,['====================================================' ...
        '==========================================\n']);
    fprintf(fid,[' S^a: Structural sensitivity index of ' ...
        'individual terms\n']);
    fprintf(fid,[' S^b: Correlative sensitivity index of ' ...
        'individual terms\n']);
...
if ispc || ismac
    edit HDMR_H_settings.txt; edit HDMR_H_results.txt;
end
