...
options = struct('graphics',1, ...      % Specify HDMR_H structure options
    'maxorder',3,'maxiter',100, ...
    'bf1',1,'m',2,'K',10,'R',300, ...
    'method',1,'alfa',0.01,'lambda',0.10, ...
    'vartol',1e-3,'refit',1); 
for t = 1:1%ny
    [results(:,:,t),Ss,Fx,Em,Xy] = ...  % Run the HDMR_H toolbox
        HDMR_H(X,Y(t,1:N)',options);                                %#ok
end
