...
# ####################################################################### #
#                                                                         #
#  High-Dimensional Model Representation (HDMR_H) for d>20 with B-spline  #
...
#  the HDMR_H model (emulator) with/without a given first, second and/or  #
...
#  (columns of X are independent), the HDMR_H sensitivity indices reduce  #
...
def HDMR_H_setup(X, y, user_options):
    """
    Setup function for HDMR_H, including validation and configuration of options.
    ...
    Returns:
    --------
    N, d, graphics, maxorder, maxiter, bf1, m, K, R, method, alfa, vartol, lambda_, refit : various configuration parameters
    """

    # Check the shape of X
    N, d = X.shape
    if d == 1:
        raise ValueError("HDMR_H ERROR: Matrix X contains only a single column: No point to do sensitivity analysis when d = 1")
    if N < 100:
        raise ValueError(f"HDMR_H ERROR: Number of samples, N ({N}), is insufficient")
    
    # Check the shape of y
    if y.shape[0] != N:
        raise ValueError(f"HDMR_H ERROR: Dimension mismatch. The number of rows, N ({N}), of y should match number of rows of X")
    ...
    with open('HDMR_H_settings.txt', 'w') as fid:
        fid.write('|-------------------------------------------------------------------------|\n')
        fid.write('|                                                                         |\n')
        fid.write('|  HHH     HHH   DDDDDDDDD    MMM     MMM   RRRRRRRR        HHH     HHH   |\n')
        fid.write('|  HHH     HHH   DDDDDDDDDD   MMM     MMM   RRRRRRRRR       HHH     HHH   |\n')
        fid.write('|  HHH     HHH   DDD    DDD   MMMM   MMMM   RRR    RRR      HHH     HHH   |\n')
        fid.write('|  HHH     HHH   DDD    DDD   MMMMM MMMMM   RRR    RRR      HHH     HHH   |\n')
        fid.write('|  HHHHHHHHHHH   DDD    DDD   MMMMMMMMMMM   RRRRRRRRR   --- HHHHHHHHHHH   |\n')
        fid.write('|  HHHHHHHHHHH   DDD    DDD   MMMMMMMMMMM   RRRRRRRR    --- HHHHHHHHHHH   |\n')
        fid.write('|  HHH     HHH   DDD    DDD   MMM     MMM   RRRRRRRRR       HHH     HHH   |\n')
        fid.write('|  HHH     HHH   DDD    DDD   MMM     MMM   RRR   RRR       HHH     HHH   |\n')
        fid.write('|  HHH     HHH   DDDDDDDDDD   MMM     MMM   RRR    RRR      HHH     HHH   |\n')
        fid.write('|  HHH     HHH   DDDDDDDDD    MMM     MMM   RRR     RRR     HHH     HHH   |\n')
        fid.write('|                                                                         |\n')
        ...
        if graphics not in [0, 1]:
            raise ValueError('HDMR_H ERROR: Field "graphics" of options should take on the value of 0 or 1')

        maxorder = int(user_options.get('maxorder', def_options['maxorder']))
        if maxorder not in [1, 2, 3]:
            raise ValueError('HDMR_H ERROR: Field "maxorder" of options should be an integer with values of 1, 2 or 3')
    
        if d == 2 and maxorder > 2:
            print("HDMR_H WARNING: Field 'maxorder' of options set to 2 as d = 2 (X has two columns)")
            maxorder = 2

        maxiter = int(user_options.get('maxiter', def_options['maxiter']))
        if not (1 <= maxiter <= 1000):
            raise ValueError('HDMR_H ERROR: Field "maxiter" of options should be an integer between 1 and 1000')

        bf1 = int(user_options.get('bf1', def_options['bf1']))
        if bf1 not in [0, 1]:
            raise ValueError('HDMR_H ERROR: Field "bf1" of options should be an integer with value 0 or 1')

        m = int(user_options.get('m', def_options['m']))
        if not (1 <= m <= 10):
            raise ValueError('HDMR_H ERROR: Field "m" of options should be an integer between 1 and 10')

        K = int(user_options.get('K', def_options['K']))
        if not (1 <= K <= 500):
            raise ValueError('HDMR_H ERROR: Field "K" of options should be an integer between 1 and 500')

        R = int(user_options.get('R', def_options['R']))
        if not (100 <= R <= N):
            raise ValueError(f'HDMR_H ERROR: Field "R" of options should be an integer between 100 and N ({N}), the number of rows matrix X')

        method = int(user_options.get('method', def_options['method']))
        if method not in [1, 2]:
            raise ValueError('HDMR_H ERROR: Field "method" of options should take on the value of 1 (forward selection) or 2 (backward elimination)')

        alfa = user_options.get('alfa', def_options['alfa'])
        if isinstance(alfa, str):
            raise ValueError('HDMR_H ERROR: Field "alfa" (confidence interval) of options should not be a string but a numerical value')
        if alfa > 0.1 or alfa < np.finfo(float).eps:
            raise ValueError('HDMR_H ERROR: Field "alfa" (confidence interval) of options should not exceed 0.1 or be smaller than eps')

        lambda_ = user_options.get('lambda', def_options['lambda'])
        if isinstance(lambda_, str):
            raise ValueError('HDMR_H ERROR: Field "lambda" (regularization term) of options should not be a string but a numerical value')
        if lambda_ > 10:
            print("HDMR_H WARNING: Field 'lambda' of options set rather large. Default: lambda = 0.1")
            lambda_ = 0.1
        if lambda_ < 0:
            raise ValueError('HDMR_H ERROR: Field "lambda" (regularization term) of options cannot be smaller than zero')

        vartol = user_options.get('vartol', def_options['vartol'])
        if isinstance(vartol, str):
            raise ValueError('HDMR_H ERROR: Field "vartol" (convergence threshold) of options should not be a string but a numerical value')
        if not (0 < vartol < 0.1):
            raise ValueError('HDMR_H ERROR: Field "vartol" (convergence threshold) of options should be between 0 and 0.1')

        refit = int(user_options.get('refit', def_options['refit']))
        if refit not in [0, 1]:
            raise ValueError('HDMR_H ERROR: Field "refit" of options should take on the value of 0 or 1')
    ...
    return N, d, graphics, maxorder, maxiter, bf1, m, K, R, method, alfa, vartol, lambda_, refit


def HDMR_H_initialize(X, y, N, d, K, R, m, maxorder):
    # Set random seed
    ...
    if maxorder == 3:
        gamma = [p for p in itertools.product(range(0, m1), repeat=3)]
    ...
    # Print to screen
    print('  --------------------------------------------------------------------       ')
    print("  HHH     HHH   DDDDDDDDD    MMM     MMM   RRRRRRRR        HHH     HHH       ")
    print("  HHH     HHH   DDDDDDDDDD   MMM     MMM   RRRRRRRRR       HHH     HHH       ")
    print("  HHH     HHH   DDD    DDD   MMMM   MMMM   RRR    RRR      HHH     HHH       ")
    print("  HHH     HHH   DDD    DDD   MMMMM MMMMM   RRR    RRR      HHH     HHH       ")
    print("  HHHHHHHHHHH   DDD    DDD   MMMMMMMMMMM   RRRRRRRRR   --- HHHHHHHHHHH     /^ ^\     ")
    print("  HHHHHHHHHHH   DDD    DDD   MMMMMMMMMMM   RRRRRRRR    --- HHHHHHHHHHH    / 0 0 \    ")
    print("  HHH     HHH   DDD    DDD   MMM     MMM   RRRRRRRRR       HHH     HHH    V\ Y /V    ")
    print("  HHH     HHH   DDD    DDD   MMM     MMM   RRR   RRR       HHH     HHH     / - \     ")
    print("  HHH     HHH   DDDDDDDDDD   MMM     MMM   RRR    RRR      HHH     HHH    /     |    ")
    print("  HHH     HHH   DDDDDDDDD    MMM     MMM   RRR     RRR     HHH     HHH    V__) ||    ")
    print('  --------------------------------------------------------------------               ')
    ...
    return Xy, Em, SA, RT, Y_em, beta, gamma, m1, m2, m3, j1, j2, j3, it2, it3, itr


def HDMR_H_1st(B1, y_res, R, n1, m1, vartol, lambda_val, maxiter, bf):
...
def HDMR_H_2nd(B1, y_res, R, beta, n2, m2, c2, lambda_val):
    ...
    for j in range(n2):
        ...
        for z in range(m2):
            B2[:R, z] =  B1[:R, beta[z][0], c2[j, 0]] * B1[:R, beta[z][1], c2[j, 1]]

        # Regularized least squares inversion (minimize || C2 ||_2)
        B22 = np.dot(B2[:R, :m2].T, B2[:R, :m2])
        cond_number = np.linalg.cond(B22)
        if cond_number > 1e15:
            pass
        else:
            T2[:m2, :R] = np.linalg.solve(B22 + lambda_val * np.eye(m2), B2[:R, :m2].T)
        C2[:m2, j] = np.dot(T2[:m2, :R], y_res)
        y_ij[:R, j] = np.dot(B2[:R, :m2], C2[:m2, j])

    return y_res, y_ij, C2, it


def HDMR_H_3rd(B1, y_res, R, gamma, n3, m3, c3, lambda_val):
    # Initialize coefficients, third-order contributions, temporary matrix, and iteration counter
    B3 = np.zeros((R, m3))      # B3 matrix
    C3 = np.zeros((m3, n3))     # Coefficients
    y_ijk = np.zeros((R, n3))   # Third-order contributions
    T3 = np.zeros((m3, R))      # Temporary matrix for third-order terms
    it = 0                      # Iteration counter
    ...
    for j in range(n3):
        # Compute B-spline coefficients
        for z in range(m3):
            B3[:R, z] =  B1[:R, gamma[z][0], c3[j, 0]] * B1[:R, gamma[z][1], c3[j, 1]] * B1[:R, gamma[z][2], c3[j, 2]] 

        # Regularized least squares inversion (minimize || C3 ||_2)
        B33 = np.dot(B3[:R, :m3].T, B3[:R, :m3])
        cond_number = np.linalg.cond(B33)
        if cond_number > 1e15:
            pass
        else:
            T3[:m3, :R] = np.linalg.solve(B33 + lambda_val * np.eye(m3), B3[:R, :m3].T)
        C3[:m3, j] = np.dot(T3[:m3, :R], y_res)
        y_ijk[:R, j] = np.dot(B3[:R, :m3], C3[:m3, j])

    return y_ijk, C3, it


def HDMR_H_F_test(Y, f0, Y_bf, R, alfa, m1, m2, m3, n1, n2, n3, n, method):
    # Initialize ind with zeros (all terms insignificant)
    ind = np.zeros(n)
    
    # Determine the significant components of the HDMR_H model via the F-test
    ...
    # Now return number of terms of final HDMR_H model
    nterms = np.sum(ind)
    
    return ind, nterms, p0


def HDMR_H_refit(B1, T1, C1, C2, C3, y_res, Y_em, R, beta, gamma, n1, n2, n3, n, c2, c3, m1, m2, m3, j1, j2, j3, ind, lambda_, vartol, maxiter, maxorder):
    ...
    i1 = np.where(ind[j1] == 0)[0]              # index insignificant 1st order terms
    C1[:m1, i1] = 0
    Y_em[:R, j1[i1]] = 0                        # set id columns to zero
    varb_old[j1] = np.sum(C1**2, axis=0)        # compute old sum coefficients
    ...
    # Initialize matrices B2 and B3
    B2 = np.zeros((R,m2))
    B3 = np.zeros((R,m3))
    
    # Backfitting of all orders combined
    while (varb_max > vartol) and (iter_count < maxiter):
        varb_new = np.zeros(n)
        ct_2 = 0
        ct_3 = 0
        ...
        for i in range(len(id)):
            ...
            j = id[i]
            ii = id.copy()
            ii = np.delete(ii, i)  # Remove current index i
            
            # Now remove all non-zero columns from Y_em except column j
            # Y_new = y_res[:, 0] - np.sum(Y_em[:, ii], axis=1)
            y_nzl = y_res - np.sum(Y_em[:, ii], axis=1)

            if j in j1:
                C1[:m1, j] = np.dot(T1[:m1, :R, j], y_nzl[:R])
                Y_em[:R, j] = np.dot(B1[:R, :m1, j], C1[:m1, j])
                varb_new[j] = np.sum(C1[:m1, j]**2)
            elif j in j2:
                z = j - n1
                if iter_count == 0:
                    for q in range(m2):
                        B2[:R, q] = B1[:R, beta[q][0], c2[z, 0]] * B1[:R, beta[q][1], c2[z, 1]]
                    # T2[:m2, :R] = np.linalg.solve(np.dot(B2[:R, :m2].T, B2[:R, :m2]) + lambda_ * np.eye(m2), B2[:R, :m2].T)
                    T2 = np.linalg.solve(np.dot(B2[:R, :m2].T, B2[:R, :m2]) + lambda_ * np.eye(m2), B2[:R, :m2].T)
                # C2[:m2, z] = np.dot(T2[:m2, :R], y_nzl[:R])
                C2[:m2, z] = np.dot(T2, y_nzl[:R])
                Y_em[:R, j] = np.dot(B2[:R, :m2], C2[:m2, z])
                varb_new[j] = np.sum(C2[:m2, z]**2)
                ct_2 += 1
            else:
                z = j - n1 - n2
                if iter_count == 0:
                    for q in range(m3):
                        B3[:R, q] = B1[:R, gamma[q, 0], c3[z, 0]] * B1[:R, gamma[q, 1], c3[z, 1]] * B1[:R, gamma[q, 2], c3[z, 2]]
                    # T3[:m3, :R] = np.linalg.solve(np.dot(B3[:R, :m3].T, B3[:R, :m3]) + lambda_ * np.eye(m3), B3[:R, :m3].T)
                    T3 = np.linalg.solve(np.dot(B3[:R, :m3].T, B3[:R, :m3]) + lambda_ * np.eye(m3), B3[:R, :m3].T)
                # C3[:m3, z] = np.dot(T3[:m3, :R], y_nzl[:R])
                C3[:m3, z] = np.dot(T3, y_nzl[:R])
                Y_em[:R, j] = np.dot(B3[:R, :m3], C3[:m3, z])
                varb_new[j] = np.sum(C3[:m3, z]**2)
                ct_3 += 1
    ...
    return Y_em, C1, C2, C3, iter_count
...
def ANCOVA(y, Y_em, V_y, R, n):
    ...
    - Y_em: Rxn matrix with HDMR_H model terms from backfitting
    - V_y: scalar, variance of the model output
    - R: number of samples of X
    - n: number of terms of HDMR_H model
...
def HDMR_H_end(SA, nterms, p0, RMSE, select, K, C2, C3, n1, n2, n3, n):
    """
    Prepares the return arguments of HDMR_H (High Dimensional Model Representation)
    ...
    with open('HDMR_H_results.txt', 'w+') as fid:
        fid.write('|--------------------------------------------------------------------------|\n')
        fid.write('|                                                                          |\n')
        fid.write('|   HHH     HHH   DDDDDDDDD    MMM     MMM   RRRRRRRR        HHH     HHH   |\n')
        fid.write('|   HHH     HHH   DDDDDDDDDD   MMM     MMM   RRRRRRRRR       HHH     HHH   |\n')
        ...
        fid.write('|   HHH     HHH   DDD    DDD   MMMMM MMMMM   RRR    RRR      HHH     HHH   |\n')
        fid.write('|   HHHHHHHHHHH   DDD    DDD   MMMMMMMMMMM   RRRRRRRRR   --- HHHHHHHHHHH   |     /^ ^\      \n')
        fid.write('|   HHHHHHHHHHH   DDD    DDD   MMMMMMMMMMM   RRRRRRRR    --- HHHHHHHHHHH   |    / 0 0 \     \n')
        fid.write('|   HHH     HHH   DDD    DDD   MMM     MMM   RRRRRRRRR       HHH     HHH   |    V\ Y /V     \n')
        fid.write('|   HHH     HHH   DDD    DDD   MMM     MMM   RRR   RRR       HHH     HHH   |     / - \      \n')
        fid.write('|   HHH     HHH   DDDDDDDDDD   MMM     MMM   RRR    RRR      HHH     HHH   |    /     |     \n')
        fid.write('|   HHH     HHH   DDDDDDDDD    MMM     MMM   RRR     RRR     HHH     HHH   |    V__) ||     \n')
        fid.write('|                                                                          |\n')
        fid.write('|--------------------------------------------------------------------------|\n')
        fid.write('\n')
        fid.write('  © Jasper A. Vrugt, University of California Irvine \n')

        # Table header based on number of bootstrap trials
        if K == 1:
            fid.write(f'Table 1: Properties of HDMR_H emulator, y = f(x), for training data set (no bootstrapping)\n')
        else:
            fid.write(f'Table 1: Properties of HDMR_H emulator, y = f(x), for randomized training data set of {K} bootstrap trials\n')
        fid.write('=========================================\n')
        ...
        for pr_tab in range(1, 3):
            ...
            if K == 1:
                if pr_tab == 1:
                    fid.write(f'Table 2: HDMR_H results for all model components using all X and Y data (no bootstrapping)\n')
                else:
                    fid.write(f'Table 3: HDMR_H results for significant model components only using all X and Y data (no bootstrapping)\n')
            elif K > 1:
                if pr_tab == 1:
                    fid.write(f'Table 2: HDMR_H results for all model components using {K} bootstrap trials\n')
                else:
                    fid.write(f'Table 3: HDMR_H results for significant model components only using {K} bootstrap trials\n')
    ...
    if sys.platform == "win32" or sys.platform == "darwin":
        os.system("start HDMR_H_settings.txt")
        os.system("start HDMR_H_results.txt")
...
def HDMR_H_plot(SA_sig, Fx, y, y_e, select, p0, iter, id, R, K, refit, maxorder):

    # Output writing to the screen
    print('\n')
    print('HDMR_H PLOTTING: PLEASE WAIT ...')
    ...
    with PdfPages(file_name) as pdf:
        ...
        plt.gca().set_xticks([])
        plt.gca().set_yticks([])
        plt.text(0.303 * x_mult, 0.6 * y_mult, r'${\rm Visual \; results \; of \; HDMR_H \; toolbox}$', fontsize=fontsize_titlepage) #, ha='center', va='center')
        plt.text(0.3 * x_mult, 0.5 * y_mult, r'${\rm Tables \; are \; printed \; to \; this \; PDF \; file}$', fontsize=fontsize_titlepage) #, ha='center', va='center') #, fontweight='bold')
...
#            plt.legend(['first order', 'second order', 'third order', 'refitting'], fontsize=fontsize_legend)
        ...
        # SA_sig_df = pd.DataFrame(SA_sig, columns=[SA_sig[0, i] for i in range(0, SA_sig.shape[1])])    used in HDMR_H
